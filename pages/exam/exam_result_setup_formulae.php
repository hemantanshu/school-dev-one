...
$body->startBody ( 'exam', 'LMENUL165', 'Result Setup Formulae' );

$resultDisplayId = $_GET['resultDisplayId'];

$resultDisplayDetails = $body->getTableIdDetails($resultDisplayId);
$resultSetupDetails = $body->getTableIdDetails($resultDisplayDetails['result_display_id']);
$resultDetails = $body->getTableIdDetails($resultSetupDetails['result_id']);
$sessionDetails = $body->getTableIdDetails($resultDetails['session_id']);

$examinationIds = $examination->getExaminationForResultPreparation($sessionDetails['id']);
?>
<input type="hidden" name="resultId" id="resultId" value="<?php echo $resultDetails['id'];  ?>" />
<input type="hidden" name="sessionId" id="sessionId" value="<?php echo $sessionDetails['id'];  ?>" />
...
<div class="display">
    <div id="displaySubjectRecord">
        <fieldset class="displayElements">
            ...
            <dl>
                <dt style="width: 15%;">
                    <label for="resultDisplayName">Display Name :</label>
                </dt>
                <dd style="width: 30%">
                    <span id="resultDisplayName"><?php echo $resultSetupDetails['display_name']; ?></span>
                </dd>
                <dt style="width: 15%">
                    <label for="displayOrder">Display Order :</label>
                </dt>
                <dd style="width: 30%">
                    <span id="displayOrder"><?php echo $resultSetupDetails['display_order']; ?></span>
                </dd>
            </dl>
        </fieldset>
    </div>
                    ...
                    <select name="examinationGroup" id="examinationGroup" class="required"  title="Select Examination Group"  tabindex="2" onchange="javascript: valid.validateInput(this);" style="width: 200px">
                    	<?php 
                    		...
                    		foreach ($examinationIds as $examinationId){
                    			...
                    			$optionValues .=  "<option value=\"$examinationId\">".$details['examination_name']."</option>";
                    		}
                    		echo $optionValues;
                    	?>
                    </select>
