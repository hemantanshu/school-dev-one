...
$result->isRequestAuthorised4Form ( 'LMENUL117' );
...
if (isset ( $_POST ['task'] )) {
	if ($_POST ['task'] == 'confirmData') {
		...
		$dataId = $result->setResultSectionData($resultSectionId, $candidateId, $data, 'HIGHT');		
		$outputArray[0] = $dataId;
		echo json_encode ( $outputArray );
	} elseif($_POST['task'] == "finalConfirmation"){
		$resultSectionId = $_POST ['resultSectionId'];
		$sectionId = $_POST['sectionId'];
		$status = $result->checkDataFillingCompletionStatus($resultSectionId, $sectionId, 'HIGHT');
		if($status){
			//closing the weight submission flag		
			$junior->updateTableParameter('height_actual_date', $junior->getCurrentDateTime());
			$junior->commitSectionDetailsUpdate($resultSectionId);
						
			//closing the task assignment 
			$menuTaskId = $menuTask->getMenuTaskId4SourceId($resultSectionId.'-H');
			...
			$notificationId = $notification->getNotificationId4Source($resultSectionId."-H");
			$notification->setUpdateLog('Complete Flag To Y');
