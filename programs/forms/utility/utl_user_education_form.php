...
$options->isRequestAuthorised4Form('LMENUL40');

if (isset ( $_POST ['task'] )) {
	if ($_POST ['task'] == 'insertRecord') {
		$userId = $_POST ['userId'];
		$outputArray [0] = 0;
		$educationId = $education->setUserEducationDetails ( $userId, $_POST ['institute_val'], $_POST ['year'], $_POST ['level'], $_POST ['score'], $_POST ['markType'] );
		if ($educationId) {
		...
		$ids = $education->getUserEducationIds ( $userId, $searchType );
		$outputArray [0] [0] = 1;
		$i = 0;
		foreach ( $ids as $id ) {
