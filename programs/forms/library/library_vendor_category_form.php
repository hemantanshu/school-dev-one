...
$vendor->isRequestAuthorised4Form ( 'LMENUL141' );

if (isset ( $_POST ['task'] )) {
	if ($_POST ['task'] == 'insertRecord') {
		$vendorId = $_POST['vendorId'];
		$categoryId = $_POST ['category_val'] == '' ? $options->setNewOptionValue ( $_POST ['category'], 'LICAT' ) : $_POST ['category_val'];	
		$vendorCategoryId = $vendor->setVendorCategory($vendorId, $categoryId);
		
		$outputArray = array ();
		$outputArray [0] = 0;
		if ($vendorCategoryId) {
			$outputArray [0] = $vendorCategoryId;
			$outputArray [] = $_POST ['category'];
		}
		echo json_encode ( $outputArray );
	} else if ($_POST ['task'] == 'search') {
		$vendorId = $_POST['vendorId'];
		$search_type = htmlentities ( trim ( $_POST ['search_type'] ) );
		$data = $vendor->getVendorCategoryIds($vendorId, $search_type);
		$i = 0;
		$outputArray [0] [0] = 1;
		foreach ( $data as $id ) {
			$details = $vendor->getTableIdDetails ( $id );
			$outputArray [$i] [0] = $details ['id'];
			$outputArray [$i] [] = $options->getOptionIdValue($details ['category_id']);			
			++ $i;
		}
		echo json_encode ( $outputArray );
		...
		$outputArray [] = $options->getOptionIdValue($details ['category_id']);
		...
		$id = htmlentities ( trim ( $_POST ['id'] ) );
		$vendor->dropVendorCategory($id);
		$outputArray [0] = $id;
		echo json_encode ( $outputArray );
	} elseif ($_POST ['task'] == 'activateRecord') {
		$id = htmlentities ( trim ( $_POST ['id'] ) );
		$vendor->activateVendorCategory($id);
		$outputArray [0] = $id;
		echo json_encode ( $outputArray );
	} else {
		$outputArray [0] = 0;
