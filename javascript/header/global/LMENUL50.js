...
function processInsertForm() {
    ...
    if (parentMenu != "") {
        ...
        showLoading("Fetching Data From Server");
        $.ajax({
            url:formGlobalPath + "global/glb_menu_parent_form.php",
            type:"POST",
            data:data,
            cache:false,
            dataType:'json',
                ...
                if (output[0] != 0) {
                    ...
                    oTable.fnAddData(
                        [
                            childMenuName,
                            output[1],
                            "<button type=\"button\" id=\"button\" class=\"regular view\" onclick=\"showMenuIdDetails('" + output[2] + "', '" + aPos + "', '" + output[0] + "')\">" + viewImageLink + "Show Details</button>"
                        ]);
                    hideInsertForm()
                    $('#insertReset').click();
                    hideInsertFormDetails();
                    ...
                    for (i = 0; i < data.length; i++) {
                        oTable.fnAddData(
                            [
                                childMenuName,
                                data[i][1],
                                "<button type=\"button\" id=\"button\" class=\"regular\" onclick=\"showMenuIdDetails('" + data[i][2] + "', '" + i + "', '" + data[i][0] + "')\">" + viewImageLink + "Show Details</button>"
                            ]);
                    }
                    hideInsertForm();
                    endLoading();
