...
$(document)
    ...
    function () {
        ...
        gradeNameGlobal = $('#gradeNameGlobal').val();
        gradeGlobal = $('#gradeGlobal').val();
        getSearchResults();
    });
...
// preparing for ajax call
    ...
    data = 'gradeId='+gradeGlobal+'&task=insertRecord&' + data;
    showLoading("Uploading Data To Server");
    $
        .ajax({
            url:formGlobalPath + "exam/exam_grading_options_form.php",
            type:"POST",
            data:data,
            cache:false,
            dataType:'json',
...
function getSearchResults() {
    ...
    data = 'gradeId='+gradeGlobal+'&task=search&' + data;
...
function showRecordDetails(id, aPos) {
    ...
    $.ajax({
        ...
        success:function (data) {
            checkValidityOfOutput(data);
            if (data[1] != '') {
                $('#gradeOption_d').html(data[1]);
                $('#gradeWeight_d').html(data[2]);
...
function editRecord(id, aPos) {
    ...
    $.ajax({
        ...
        success:function (data) {
            checkValidityOfOutput(data);
            if (data[0] != 0) {
                $('#gradeOption_u').val(data[1]);
                $('#gradeWeight_u').val(data[2]);
                if (data[7] == 'y') {
                    $('#dropRecord_u').show();
                    $('#dropRecord_u')
                        .attr(
            ...
            .ajax({
                ...
                success:function (output) {
                    ...
                    if (output[0] != 0) {
                        ...
                        handleNotification(
                            'Error In Dropping Grade Name ... Try After Sometime',
                            'error');
                        endLoading();
                    }
            ...
            .ajax({
                ...
                success:function (output) {
                    ...
                    if (output[0] != 0) {
                        ...
                        handleNotification(
                            'Error In Activating Grade Option Type ... Try After Sometime',
                            'error');
                        endLoading();
                    }
