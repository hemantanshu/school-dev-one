            ...
            success:function (data) {
                ...
                if (data[0] != 0) {
                    ...
                    oTable
                        ...
                        data[1],
                        data[2],
                        "<button type=\"button\" id=\"details\" class=\"positive details\" onclick=\"setUrls('"
                            + data[0]
                            + "', '"
                            + aPos
                            + "')\">" + browseImageLink + "URLS</button>",
                        "<button type=\"button\" id=\"details\" class=\"negative details\" onclick=\"setFields('"
                            + data[0]
                            + "', '"
                            + aPos
                            + "')\">" + browseImageLink + "Fields</button>",
                        "<button type=\"button\" id=\"details\" class=\"regular details\" onclick=\"showRecordDetails('"
                            + data[0]
                            + "', '"
                            + aPos
                            + "')\">" + viewImageLink + "Show</button>",
...
function showRecordDetails(id, aPos) {
    ...
    $.ajax({
        ...
        success:function (data) {
            checkValidityOfOutput(data);
            if (data[0] != 0) {
                $('#resultType_d').html(data[1]);
                $('#resultOrder_d').html(data[2]);
                $('#description_d').html(data[3]);
...
function editRecord(id, aPos) {
    ...
    $.ajax({
        ...
        success:function (data) {
            checkValidityOfOutput(data);
            if (data[0] != 0) {
                $('#resultType_u').val(data[1]);
                $('#resultOrder_u').val(data[2]);
                $('#resultDescription_u').val(data[3]);
                if (data[8] == 'y') {
                    $('#dropRecord_u').show();
                    $('#dropRecord_u')
                        .attr(
...
function setFields(id, aPos){
    var url = serverUrl+'pages/exam/exam_result_type_fields.php?resultTypeId='+id;
    loadPageIntoDisplay(url);
    return false;
}

function setUrls(id, aPos){
    var url = serverUrl+'pages/exam/exam_result_type_urls.php?resultTypeId='+id;
    loadPageIntoDisplay(url);
    return false;
}
