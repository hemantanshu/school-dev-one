...
$(document)
    ...
    function () {
        ...
        $("#organization").result(function (event, data, formatted) {
            $("#organization_val").val(data[1]);
        });

        $("#position")
            .autocomplete(
            formGlobalPath
                + "global/glb_option_search.php?type=RANKS&option=yes",
            {
                width:260,
                matchContains:true,
                mustMatch:false,
        ...
        $("#position").result(function (event, data, formatted) {
            $("#position_val").val(data[1]);
        });
        ...
        $("#organization_u").result(
            function (event, data, formatted) {
                $("#organization_uval").val(data[1]);
            });
        ...
        $("#position_u").result(function (event, data, formatted) {
            $("#position_uval").val(data[1]);
        });
...
function editRecord(id, aPos) {
    ...
    $.ajax({
        ...
        success:function (data) {
            checkValidityOfOutput(data);
            if (data[0] != 0) {
                $('#organization_u').val(data[1]);
                $('#organization_uval').val(data[11]);
                $('#position_u').val(data[2]);
                $('#position_uval').val(data[12]);
                $('#startDate_u').val(data[13]);
                $('#endDate_u').val(data[14]);
            ...
            .ajax({
                ...
                success:function (output) {
                    ...
                    if (output[0] != 0) {
                        ...
                        handleNotification(
                            'Error In Dropping Employment Details ... Try After Sometime',
                            'error');
                        endLoading();
                    }
            ...
            .ajax({
                ...
                success:function (output) {
                    ...
                    if (output[0] != 0) {
                        ...
                        handleNotification(
                            'Error In Activating Employment Details ... Try After Sometime',
                            'error');
                        endLoading();
                    }
