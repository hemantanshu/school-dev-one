...
function processInsertForm() {
    ...
    $
        .ajax({
            url:formGlobalPath + "library/library_vendor_entry_form.php",
            type:"POST",
            data:data,
            cache:false,
            dataType:'json',
...
function showRecordDetails(id, aPos) {
    ...
    $.ajax({
        ...
        success:function (data) {
            checkValidityOfOutput(data);
            if (data[0] != 0) {
                $('#vendorName_d').html(data[1]);
                $('#weightage_d').html(data[2]);
                $('#contactNo_d').html(data[3]);
                $('#emailId_d').html(data[4]);
                $('#address_d').html(data[5]);
                $('#category_d').html(data[6]);
                $('#tag_d').html(data[7]);
                ...
                $('#categoryButton').attr('onclick',
                    'categoryRecord(\'' + data[0] + '\', \'' + aPos + '\')');

                $('#tagButton').attr('onclick',
                    'tagRecord(\'' + data[0] + '\', \'' + aPos + '\')');
...
function editRecord(id, aPos) {
    ...
    $.ajax({
        ...
        success:function (data) {
            checkValidityOfOutput(data);
            if (data[0] != 0) {
                $('#vendorName_u').val(data[1]);
                $('#weightage_u').val(data[2]);
                $('#contactNo_u').val(data[3]);
                $('#emailId_u').val(data[4]);

                $('#streetAddress1_u').val(data[13]);
                $('#streetAddress2_u').val(data[14]);
                $('#city_uval').val(data[16]);
                $('#city_u').val(data[15]);
                $('#state_uval').val(data[18]);
                $('#state_u').val(data[17]);
                $('#pincode_u').val(data[19]);
                $('#country_uval').val(data[21]);
                $('#country_u').val(data[20]);
...
function categoryRecord(id, aPos){
    var url = serverUrl+'pages/library/library_vendor_category.php?vendorId='+id;
    loadPageIntoDisplay(url);
    return false;
}

function tagRecord(id, aPos){
    var url = serverUrl+'pages/library/library_vendor_tag.php?vendorId='+id;
    loadPageIntoDisplay(url);
    return false;
}